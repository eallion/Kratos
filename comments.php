...
<?php if($this->allow('comment')): ?>
	...
	<form action="<?php $this->commentUrl() ?>" method="post" id="commentform" class="comment-form">
		...
		<?php else: ?>
		<p class="comment-notes"><span id="email-notes">电子邮件地址不会被公开，</span> <span class="required">*</span> 为必填项，必须包含中文，不能提交URL。</p><?php endif; ?>
		<div class="comment form-group has-feedback"><div class="input-group"><textarea class="form-control" id="comment" placeholder="说点什么吧…… " name="text" rows="5" aria-required="true" required="" onkeydown="if(event.ctrlKey){if(event.keyCode==13){document.getElementById('submit').click();return false}};"></textarea></div></div><?php if(!$this->user->hasLogin()): ?>
